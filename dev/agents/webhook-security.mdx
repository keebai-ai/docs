...
description: "Which URLs a function may target, how header and query-string secrets are masked, and what the validator does not protect against."
---
...
Keebai has other kinds of function — calendar integrations, ecommerce lookups, the ones generated automatically from a `product_info` block. None of them appear here. [`GET /v1/tools`](/dev/endpoints/tools-list) filters to webhooks, and asking for a non-webhook by id returns `404` rather than `403`, so an id you cannot manage is indistinguishable from one that does not exist.
...
<CardGroup cols={2}>
  <Card title="Create a function" icon="plus" href="/dev/endpoints/tools-create">
    The full body, with parameters and placeholders.
  </Card>
  <Card title="Update a function" icon="pen" href="/dev/endpoints/tools-update">
