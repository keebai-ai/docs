...
| `project_id` | string \| absent | Project the event belongs to. Present on events that originate inside one. |
...
<Warning>
  `dev_tenant_id` is an **internal identifier** and ships only on `whatsapp.message.received`
  for channels onboarded through the reseller flow. It is documented here because it is on the
