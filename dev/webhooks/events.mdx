...
title: "Event catalog"
sidebarTitle: "Events"
description: "The 36 events you can subscribe to, across messaging, Flows, conversations, CRM, scheduling, and ecommerce. Each event has its own `data` shape."
---
...
- **`crm.ticket.updated` also fires when nobody edited the ticket.** [Emitting a CRM event](/dev/crm/events#values-are-written-through) whose type declares ticket-scoped fields writes those values onto the ticket, and that write is an update like any other. An ingest running all day produces these all day. If you write back to tickets in response, [guard against the loop](/dev/webhooks/overview#beware-the-write-back-loop).

There is no `crm.event.*` webhook. Events are the inbound direction — to be told about them, subscribe to the ticket events they cause.
...
A new appointment was booked — from the portal, the public API, the agent, or an external calendar sync.
...
| `source` | enum | `agenda` (staff), `web` (public booking), `ia` (the agent booked it). |
...
A sale was closed — from the portal, the point of sale, or the agent.
