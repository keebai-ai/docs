...
<Warning>
  **`recovery_status` is not advanced by this API.** It is set by whoever does the following up — the portal, the assistant, or an integration writing through the platform. Reading `pending` does not mean nobody has acted; it means nothing has recorded that they did.
...
<CardGroup cols={2}>
  ...
  <Card title="Record it on the CRM" icon="bolt" href="/dev/crm/events">
    Emit a CRM event so the order shows up on the contact's timeline and the assistant knows about it.
  </Card>
</CardGroup>
