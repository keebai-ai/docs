...
`recovery_url` is the link that puts the customer back in checkout with the cart intact. Sending it over WhatsApp with [`POST /v1/messages/interactive/cta-url`](/dev/endpoints/messages-interactive-cta-url) is the intended pairing.
