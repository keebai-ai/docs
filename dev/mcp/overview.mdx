...
description: "Connect an AI agent to Keebai over the Model Context Protocol — same token, same scopes, 71 tools."
---
...
<Steps>
  <Step title="Every tool is always listed">
    `tools/list` returns all 71 tools regardless of what your token can do. The catalogue is not filtered by scope.
  </Step>
...
<Warning>
  **Nine of the 71 tools delete things permanently**, and CRM and contact reads span your whole company, not just one project. Think about that before handing an agent a token with `*:delete` scopes — the [tool catalogue](/dev/mcp/tools) marks every destructive tool.
</Warning>
...
<CardGroup cols={2}>
  <Card title="Tool catalogue" icon="wrench" href="/dev/mcp/tools">
    All 71 tools with their required scope.
  </Card>
