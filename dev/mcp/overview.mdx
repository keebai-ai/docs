...
description: "Connect an AI agent to Keebai over the Model Context Protocol — same token, same scopes, 104 tools."
---
...
<Steps>
  <Step title="Every tool is always listed">
    `tools/list` returns all 104 tools regardless of what your token can do. The catalogue is not filtered by scope.
  </Step>
...
<Warning>
  **Twelve of the 104 tools delete things permanently**, and CRM and contact reads span your whole company, not just one project. Think about that before handing an agent a token with `*:delete` scopes — the [tool catalogue](/dev/mcp/tools) marks every destructive tool.
</Warning>
...
<CardGroup cols={2}>
  <Card title="Tool catalogue" icon="wrench" href="/dev/mcp/tools">
    All 104 tools with their required scope.
  </Card>
