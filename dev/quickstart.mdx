...
Grab the `phone_number_id` of your channel from [`GET /v1/whatsapp/numbers`](/dev/endpoints/whatsapp-numbers), then call `POST /v1/messages/template`:
...
| `400 Bad Request` | Body is malformed or `phone_number_id` is off. | Check the JSON against the [endpoint contract](/dev/endpoints/messages-template). |
| `429 Too Many Requests` | You hit the rate limit (300 req/min, **1,000 req/day** per token). | Read `Retry-After-<window>` and wait. See [Rate limits](/dev/rate-limits). |
