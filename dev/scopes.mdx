...
| `knowledge:assign` | knowledge | [Assign nodes](/dev/endpoints/knowledge-assign) of the knowledge base to an agent's workers. |
| `agents:read` | agents | [List agents](/dev/endpoints/agents-list), [read the sections of their prompt](/dev/endpoints/agents-prompts-list) and [their history](/dev/endpoints/agents-prompt-versions-list), read [the catalogue](/dev/endpoints/prompt-types-list), and [list webhook tools](/dev/endpoints/tools-list). |
| `agents:write` | agents | [Create](/dev/endpoints/agents-create) and [configure agents](/dev/endpoints/agents-update), [write the sections of their prompt](/dev/endpoints/agents-prompt-set) and [restore older ones](/dev/endpoints/agents-prompt-version-restore), and [manage the webhook tools](/dev/endpoints/tools-create) they can call. |
...
| `crm:events:write` | crm | [Emit CRM events](/dev/endpoints/crm-events-emit) onto a contact's timeline. Opens a ticket if the contact has none, and writes the event's values onto the ticket and the contact. No read counterpart. |
...
The [MCP server](/dev/mcp/overview) does not have a scope system of its own. Each of its 104 tools requires exactly the scope its REST counterpart does — `keebai_crm_ticket_create` needs `crm:tickets:write`, `keebai_contacts_list` needs `contacts:read`, and so on. The [tool catalogue](/dev/mcp/tools) lists the mapping.
...
<CardGroup cols={2}>
  ...
  <Card title="Knowledge-base sync">
    Your CMS or docs pipeline publishes documents and assigns them to agents automatically.

    `knowledge:read`, `knowledge:write`, `knowledge:delete`, `knowledge:assign`, `agents:read`.
  </Card>
  ...
  <Card title="Agent provisioning">
    Your onboarding flow creates an agent per client and writes its prompt blocks from a template you keep in git.

    `agents:read`, `agents:write`.
  </Card>
  <Card title="Prompt auditor">
    A reviewer reads every agent's blocks and their history to check what the bots are told to say. Cannot change a word, and webhook secrets stay masked.
