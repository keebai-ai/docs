...
<Warning>
  **Only the seven free types accept content here.** `product_info`, `store_info`, `reservation_info`, `tables_info` and `tags` return `409 BLOCK_TYPE_NOT_WRITABLE`. Check `content_writable` on the block before calling, and read [why](/dev/agents/overview#catalogue-blocks-are-read-only-here) if the refusal is surprising.
</Warning>
...
- **Agents pinned to an older version do not see this.** A save moves the block's current version; an agent with `pinned_version_id` set keeps rendering the version it was pinned to until you [repin it](/dev/endpoints/agents-blocks-set).
...
- **The change reaches live conversations within about fifteen minutes at worst.** The composed prompt is cached per agent and the cache is dropped on every save, but a conversation already mid-turn finishes with the previous version.
