...
description: "List every block in the project, attached to an agent or not, with metadata and no content."
openapi: "GET /v1/blocks"
---

The project's whole block catalogue. Blocks are not owned by an agent: they live here and any number of agents can reference them, so this is where you look for something to reuse.
...
- **Archived blocks are hidden by default but still active.** `include_archived=false` only affects this listing: an archived block that is still attached to an agent keeps rendering into its prompt. To take it out of an agent, [detach it](/dev/endpoints/agents-block-detach).
