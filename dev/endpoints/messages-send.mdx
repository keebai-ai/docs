...
- **The message lands in the Keebai inbox**: every send is persisted against the recipient's contact and pushed to the portal in realtime, on all four channels. It shows up with the `agent` role, since a PAT send is an integrator acting on the tenant's behalf rather than the AI agent.
