...
The function, same shape as [`GET /v1/tools/{id}`](/dev/endpoints/tools-get), with the header values already masked.
...
- **Creating a function does not attach it to anything.** The model can only call functions bound to a block in the agent's prompt; that binding is done in the portal.
