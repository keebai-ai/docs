...
How the agent writes: message length, emoji usage, and formatting rules. Separate from what it says, so you can change the voice without touching the content.
...
<Note>
  **Identity block: one per agent.** A second `response_format` on the same agent fails with `ASSISTANT_DUPLICATE_IDENTITY_BLOCK`.
</Note>
...
- Channel-specific rules belong here. The same agent answering on WhatsApp and on web chat reads the same formatting block, so say which rule applies where.
