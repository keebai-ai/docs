...
Limits apply **per project** (not per token, not per user, not per IP) and are shared across **every endpoint** — the quota is one pool, so calls to `/v1/messages/text` and `/v1/crm/tickets` draw from the same budget.
